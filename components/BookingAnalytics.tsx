    ...
    <Card className="shadow-xl border-2 border-[var(--primary-200)]" variant="elevated">
      ...
      <CardContent className="p-4 sm:p-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
          <div className="p-4 bg-gradient-to-br from-[var(--primary-50)] to-[var(--primary-100)] rounded-xl border-2 border-[var(--primary-200)]">
            <p className="text-xs font-medium text-[var(--primary-700)] mb-1">Total Bookings</p>
            <p className="text-3xl font-bold text-[var(--primary-900)]">{totalBookings}</p>
          </div>
          <div className="p-4 bg-white border-2 border-[var(--primary-300)] rounded-xl">
          ...
          <div className="p-4 bg-white border-2 border-[var(--primary-400)] rounded-xl">
          ...
          <div className="p-4 bg-white border-2 border-[var(--primary-500)] rounded-xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          {/* Recent Activity */}
          <div className="bg-[var(--muted)] rounded-xl p-5 border-2 border-[var(--border)]">
                  ...
                  <div key={booking.id} className="p-3 bg-[var(--card)] rounded-lg border border-[var(--border)]">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-[var(--muted-foreground)]">{formatDateShort(booking.date)}</p>
                      </div>
                      <p className="font-bold text-[var(--primary-600)]">Rs. {booking.totalPrice}</p>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-[var(--muted-foreground)] text-sm">No recent bookings</p>
            )}
          </div>
            ...
            {topTimeSlots.length > 0 ? (
              <div className="space-y-2">
                {topTimeSlots.map(([slot, count], index) => (
                  <div key={slot} className="flex items-center justify-between p-3 bg-[var(--card)] rounded-lg border border-[var(--border)]">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 bg-[var(--primary-100)] rounded-lg flex items-center justify-center font-bold text-[var(--primary-700)]">
                        {index + 1}
                      </div>
                      <div>
                        <p className="font-semibold text-[var(--foreground)]">{formatTimeSlot(slot)}</p>
                        <p className="text-xs text-[var(--muted-foreground)]">{count} booking{count > 1 ? 's' : ''}</p>
                      </div>
                    </div>
                    <div className="w-16 bg-[var(--primary-100)] rounded-full h-2">
                      <div
                        className="bg-[var(--primary-600)] h-2 rounded-full"
              ...
              <p className="text-[var(--muted-foreground)] text-sm">No booking data available</p>
            )}
          </div>
        </div>
      </CardContent>
